...
if __name__ == '__main__':
    ...
    # # x = x.reshape((len(x), 1))
    # x_new = np.linspace(0.5, 10.5, 11)[:, None]
    # kern_func = GPy.kern.RBF(1, lengthscale=1., variance=0.5)
    # Kxx = kern_func.K(x[:, None]) + noise_var * np.eye(len(x))
    # y = np.random.multivariate_normal(np.zeros(len(x)),
    #                                   Kxx)
    #
    # y -= np.mean(y)
    #
    ...
    # # y = np.sin(x) + 0.05 * np.random.randn(n_x)
    #
    # m = GP(x[:, None], y[:, None], kern=kern_func, remove_y_mean=True)
    # m._lik_variance = 0.1
    # # res = m.optimize()
    # print(m)
    # # print(res)
    #
    # # print(np.allclose(np.linalg.solve(m.kern.K(m.X), m.Y), m.alpha))
    #
    # m_ref = GPy.models.GPRegression(x[:, None], y[:, None], kern_func.copy())
    # m_ref.likelihood.variance[:] = 0.1
    # # m_ref.optimize()
    # print(m_ref)
    #
    # m.plot()
    ...
    # mu_ref, var_ref = m_ref.predict(m_ref.X)
    #
    # print("mu close?", np.allclose(mu, mu_ref, atol=1e-5))
    # print("my mu", mu)
    # print("GPy mu", mu_ref)
    # print("var close?", np.allclose(var, var_ref, atol=1e-5))
    #
    # # print("m.alpha", m.alpha)
    # # print("m_ref.alpha", m_ref.likelihood.alpha)
    #
    # # # Testing pdinv
    # # import numpy as np
    # # import matplotlib.pyplot as plt
    ...
    # # Kxx = kern.K(X,X)
    # #
    # # Kinv_np = np.linalg.inv(Kxx)
    # # CholK_np = np.linalg.cholesky(Kxx)
    # # CholKinv_np = np.linalg.cholesky(Kinv_np)
    # # LogDetK_np = np.log(np.linalg.det(Kxx))
    # # Kinv_gpy, CholK_gpy, CholKinv_gpy, LogDetK_gpy =\
    # #    GPy.util.linalg.pdinv(Kxx)
    # #
    # # print np.allclose(Kinv_np, Kinv_gpy)
    # # print np.allclose(CholK_gpy, CholK_np)
    # # print np.allclose(CholKinv_gpy, CholKinv_np)
    # # print np.allclose(LogDetK_gpy, LogDetK_np)
    #
    # #  Testing predictions
    # # x_star = np.array([[5], [6]])
    # # mu1, var1 = m.predict(x_star)
    # # mu2, var2 = m_ref.predict(x_star)
    # # print mu1 - mu2
    # # print var1 - var2
    ...
    f = np.sin(2 * x)
    f -= np.mean(f)
    y = f + 0.5 * np.random.randn(*f.shape)

    kernel_mle = GPy.kern.RBF(1)
    m_mle = GP(x[:, None], y[:, None], kernel_mle)
    m_mle.optimize()

    kernel_map = GPy.kern.RBF(1)
    m_map = GP(x[:, None], y[:, None], kernel_map,
               hyper_priors=[GPy.priors.Gamma(a=1., b=0.05),
                             GPy.priors.Gamma(a=1., b=0.05),
                             GPy.priors.Gamma(a=1., b=0.05)])
    m_map.optimize()
